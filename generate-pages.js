...
<body>
  ...
  <div id="file-viewer" style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: white; padding: 20px; border-radius: 5px; width: 80%; max-height: 80%; overflow-y: auto; z-index: 1001;">
    <button onclick="closeViewer()" style="position: absolute; top: 10px; right: 10px; cursor: pointer;">✖</button>
    <pre id="file-content" style="white-space: pre-wrap; overflow-x: auto;"></pre>
  </div>
...
function generateHomePage(outputRoot) {
  ...
  if (fs.existsSync(homePagePath)) {
    logProgress('✅ Página personalizada de inicio encontrada (index.html). No se sobrescribirá.')
    return
  }
...
function processDirectory(srcPath, destPath, relativePath = '') {
  ...
  try {
    ...
    if (contents.dirs.length + contents.files.length > 0) {
      ...
      logProgress(`⚠️ No se generó index.html para directorio vacío: ${relativePath || 'raíz'}`)
    }
  } catch (error) {
    console.error(`❌ Error al procesar directorio ${srcPath}: ${error.message}`)
    if (CONFIG.debug) {
