...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function configureDownloadButton(pdfFile) {
    if (pdfFile) {
      downloadButton.style.display = 'inline-block' // Mostrar el botón
      downloadButton.href = pdfFile // Asignar la URL del archivo PDF
      downloadButton.download = pdfFile.split('/').pop() // Nombre del archivo para descargar
    } else {
      downloadButton.style.display = 'none' // Ocultar el botón si no hay archivo
    }
  }
  ...
  if (pdfFile) {
    ...
    loadPDF(pdfFile)
    configureDownloadButton(pdfFile) // Configurar el botón de descarga
  } else {
    viewer.innerHTML = 'No se ha especificado un archivo PDF para cargar'
    configureDownloadButton(null) // Ocultar el botón de descarga
  }
})
